...
let loadMenu = () => {
  let navInnerHTML = "";
  navInnerHTML = `<ul class="nav-container">`;
  for (i in pageSections) {
    navInnerHTML += `<li class="nav-el">
        <a href="#" class="nav-el__item" id="${pageSections[i].menuData.id}">${pageSections[i].menuData.aText}</a>
      </li>`;
  }
  navInnerHTML += `</ul>`;
  document.querySelector("nav").innerHTML = navInnerHTML;
};

let loadPageData = (section) => {
  let sectionData = pageSections[section].pageData;
  let imgHTMLText =
    '<img src="' +
    sectionData.imgSrc +
    '" alt="' +
